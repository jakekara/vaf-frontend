...
export default function getSourceMaterials(
    ...
    return new Promise((resolve, reject) => {
        performGET("q/related_documents/p/id/45507/results?config=full2").then((resp: any) => {
            console.log("got document list", resp)
            let result: ItemListResponse<SourceMaterial> = getEmptyQueryResponse();
            result.items = resp.items.map(convertRawItemToSourceMaterial).filter((a: any) => a !== undefined);
